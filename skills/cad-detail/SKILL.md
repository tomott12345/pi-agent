...
description: |
  ...
  headless with QCAD, and saves the result as a DWG file to ~/Documents/CAD/.
  Use when asked to draw, detail, or produce a CAD file for any engineering,
  architectural, or construction detail.
license: MIT
...
| Radius dim | `addRadiusDim(cx,cy, r, angle)` |
| Diameter dim | `addDiameterDim(cx,cy, r, angle)` |
...
`addCircle` / `addArc` / `addPolyline` / `addPolygon`. The hatch, dimension,
label, and leader helpers set their own layers automatically.
